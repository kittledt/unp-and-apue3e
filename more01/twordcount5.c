...
/*********************
    ...
    to verify my thinking , flag for cooperate,    lock2 for mutex thread t1 and t2

    think failed
    t1 finish, unlock lock2 && raise flag , t2  may run fast , but main not fetch report of t1,
...
pthread_mutex_t lock2 = PTHREAD_MUTEX_INITIALIZER;      //to mutex thread t1 and t2

pthread_cond_t  flag = PTHREAD_COND_INITIALIZER;        // to lock flag - this critical resource
...
void *count_words(void *a)
{
    ...
    pthread_mutex_lock(&lock2);	    /* get the mailbox  access, critical resource*/
	...
	pthread_mutex_unlock(&lock2);	/* release the mailbox , release critical resource */
