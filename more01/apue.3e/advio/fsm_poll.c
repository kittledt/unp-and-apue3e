/*******************
	...
	modification: use poll()  non busy-- wait  to  replace busy--wait
...
enum
{
    STATE_R=1,
    STATE_W,
    STATE_AUTO,			//add for isolate,   only if state < STATE_AUTO,  need to call select() to monitor
    STATE_Ex,
    STATE_T
};
 ...
 static void relay(int fd1,int fd2)
 {
    ...
    fsm21.dfd = fd1;
     ...
     while(fsm12.state != STATE_T || fsm21.state != STATE_T)
     {
         // 调用状态机驱动函数，状态机开始工作
         //fsm_driver(&fsm12);
         //fsm_driver(&fsm21);------------------old, always dirve fsm whatever have data,  busy---wait


		 //布置监视任务
		 //FD_ZERO(&rset);
		 //FD_ZERO(&wset);			---------select(2)
		...
		if(fsm12.state == STATE_R)
 		...
 		if(fsm21.state == STATE_W)
 			FD_SET(fsm21.dfd,&wset);				// --select(2)
		*/
		...
		//布置监视任务
 		...
 		{
 			//以阻塞形式监视,new,  no data, not drive fsm
 		...
 		//check monitor result ,  fd1 = tty1, fd2 = tty2 ,  fd1-read ok or fd2-write ok  or state>auto,  have data,then drive fsm.
