 ...
 * building: cc twebserv.c socklib.c -lpthread -o twebserv
    ...
    server run:./twebserv 12345
    client test run :
    1.list dir ---- curl ecallspirent01:12345
    3.cat file ---- curl ecallspirent01:12346/a.html
    4 status   -----curl ecallspirent01:12345/status
    5
...
main(int ac, char *av[])
{
	...
	int		*fdptr;
	pthread_t	worker;
	pthread_attr_t	attr;       //for create detached thread. no need wait thread return
    ...
    /**
        one request , one thread handle
    */
	while(1){

		fd = accept( sock, NULL, NULL );
		server_requests++;                                      // 1 based , client number

		fdptr = malloc(sizeof(int));                            //1 req, 1 malloc
		*fdptr = fd;
		pthread_create(&worker,&attr,handle_call,fdptr);        //socket fd is input for thread
...
skip_rest_of_header(FILE *fp)
{
	...
	{
        printf("skip_rest_of_header(): request = %s",  buf);
	}
...
process_rq( char *rq, int fd)
{
	...
	/********
            client                                              server
        curl ecallspirent01:12345               ->      GET / HTTP/1.1
        curl ecallspirent01:12345/script2       ->      GET /script2 HTTP/1.1



	*/

	if ( sscanf(rq, "%s%s", cmd, arg) != 2 )        //format input , src buf is request
		return;

	printf("arg = %s\n",  arg);
	sanitize(arg);                                  //parse arg, do  ls; exec; cat
	...
	if ( strcmp(cmd,"GET") != 0 )
		...
		;
	else if ( not_exist( arg ) )                    // true,  no exist a  dir orfile,  do 404,
		do_404(arg, fd);
	else if ( isadir( arg ) )
		do_ls( arg, fd );
	else
 ...
 * make sure all paths are below the current directory
    str is input string , after sanitize, it is output string
...
sanitize(char *str)
{
	...
	while( *src ){
		...
		if( strncmp(src,"/../",4) == 0 )        //4 chars  are same
        {
            printf("/../\n");
            src += 3;
        }

		else if ( strncmp(src,"//",2) == 0 )    //2 chars  are same
        {
            printf("//\n");
			src++;
        }
...
built_in(char *arg, int fd)
{
	...
	if ( strcmp(arg,"status") != 0 )                                        //  arg != status, return
		return 0;
	http_reply(fd, &fp, 200, "OK", "text/plain",NULL);                      // arg == status, do below ,
...
do_404(char *item, int fd)
{
	...
	http_reply(fd,NULL,404,"Not Found","text/plain","he item you seek is not here");
...
isadir(char *f)
{
	...
	struct stat info;
	return ( stat(f, &info) != -1 &&S_ISDIR(info.st_mode) ); //arg(f) is  valid, get info struct,  succ 0,
...
not_exist(char *f)
{
	...
	struct stat info;
	return( stat(f,&info) == -1 );      //arg(f) is  valid, get info struct,  succ 0,  return false,  fail, -1 return true
